...
def insert_data(act_name, act_mail, name, email, res_score, timestamp, no_of_pages, reco_field, cand_level, skills, recommended_skills, courses):
    DB_table_name = 'user_data'
    insert_sql = "insert into " + DB_table_name + """
    values (0,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"""
    rec_values = (act_name, act_mail, name, email, str(res_score), timestamp, str(
...
def run():
    ...
    img = Image.open('./Logo/SMART-RESUME.png')
    ...
    if choice == 'Normal User':
        ...
        if pdf_file is not None:
            ...
            if resume_data:
                ...
                insert_data((act_name), (act_mail), resume_data['name'], resume_data['email'], str(resume_score), timestamp,
        ...
        if st.button('Login'):
            if ad_user == 'admin' and ad_password == 'admin123':
                ...
                df = pd.DataFrame(data, columns=['ID', 'Name', 'Email', 'Predicted Name', 'Predicted Mail', 'Resume Score', 'Timestamp', 'Total Page',
